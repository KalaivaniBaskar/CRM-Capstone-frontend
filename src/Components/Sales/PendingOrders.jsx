...
import { ORDER_STATUS, REQUEST_STATUS } from '../../Data/statusCode';
...
const PendingOrders = () => {
    ...
    const getPendingOrders = async() => {
      try{
        ...
        const config = { headers : {"x-auth-token" : token}}
        const response = await axios.post(`${BASE_URL}/orders/get-orders`, {order_status : ORDER_STATUS.Placed}, config) 
        console.log(response) 
        if( response.status === 200) {
       ...
       <Grid container justifyContent={'center'} alignItems={'center'}
        my={'1rem'} p={'1rem'} gap={2}>
            ...
            <div className='grid-container-od' key={od.orderID}>
                ...
                onClick ={() => setReqOrder(od.orderID)}> Ship Order </Button>
                </Box>
            {/* </Stack> */}
            <Divider />
